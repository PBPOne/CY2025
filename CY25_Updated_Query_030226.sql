...
tr1 as 
	(select 
		tr.PartnerCode,tr.leadid, TotalPremium as Total_pr,tr.netpr,IsComplianceN,Marking,tr.[Insurer Name], --SalesCat,  NH_NAME,
...
cte as
	(select
		...
		Product,
		MON1,
		cast(sum(Total_pr) as int) as PR,
		sum(netpr) as net_premium,
		cast(sum(Accrual_Net) as int) as ac_net,
		cast(sum(case when IsComplianceN= 'yes' and Marking is null then Accrual_Net else 0 end) as int) as ac_net_c
	from tr1
	--where [Insurer Name] in ('LIC India')
