with dates as 
			(select '2025-01-01' as min_date, 
					'2025-04-01' as max_date),
spl_deals as
(
	select 
...
from [PospDB].[dbo].BookingDetails_v1 (nolock)
		where ProductID in ('3','106','118','130','138','144','189','190','147')
),
life_plans as --pl
(
select 
...
(
	...
	case when vw.ProductId in ('3','106','118','130','138','144','189','190','147','224')		--Health
	    and vw.BookingDate < '2025-09-22'   --added 2811 Health GST free
		--and vw.BusinessType not in ('Renewal','Renew')       --Removed on 2811
		then round(convert(float,vw.TotalPremium)/1.18,0) 
		else vw.APE end as netpr,
...
from all_bookings vw
	...
	or (
	vw.ProductId in ('3','106','118','130','138','144','189','190','147') --Health
...
tr1 as 
	(select 
		...
		--format(tr.BookingDate,'MMM-yyyy') AS MON,
		--format(tr.BookingDate,'yyyy-MM') AS MON1,
		DATEFROMPARTS(YEAR(BookingDate), MONTH(BookingDate), 1) as MON1,
		case 
			when tr.ProductId in ('3','106','118','130','138','144','189','190','147') then 'Health' 
		...
		case 
			when tr.[Insurer Name] in ('LIC India') then 0
			...
			when tr.product_updated in ('3','118','189','130','147','1900') then tr.netpr*1.25   ---added '3-travel' 
